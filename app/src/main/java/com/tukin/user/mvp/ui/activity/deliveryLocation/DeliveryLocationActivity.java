...
import com.tukin.user.mvp.di.others.AutoCompleteAdapter;
...
public class DeliveryLocationActivity extends BaseActivity implements DeliveryLocationMvpView, DatePickerDialog.OnDateSetListener, TimePickerDialog.OnTimeSetListener {
    ...
    @BindView(R.id.iv_asap_tick)
    AppCompatImageView iv_asap_tick;

    @BindView(R.id.iv_schedule_tick)
    AppCompatImageView iv_schedule_tick;

    @BindView(R.id.ll_schedule_layout)
    LinearLayout ll_schedule_layout;
    ...
    @OnClick(R.id.rl_asap)
    void onClickRlAsap(View v) {

        iv_asap_tick.setVisibility(View.VISIBLE);
        iv_schedule_tick.setVisibility(View.GONE);
        ll_schedule_layout.setVisibility(View.GONE);

        // Asap
        locationSession.setOrderWhen("0");

    }

    @OnClick(R.id.rl_schedule)
    void onClickRlSchedule(View v) {

        iv_asap_tick.setVisibility(View.GONE);
        iv_schedule_tick.setVisibility(View.VISIBLE);
        ll_schedule_layout.setVisibility(View.VISIBLE);

        // Schedule
        locationSession.setOrderWhen("1");

    }

    @OnClick(R.id.ll_schedule_date)
    void onClickSelectDate() {
        Calendar calendar = Calendar.getInstance();
        DatePickerDialog dpd = DatePickerDialog.newInstance(DeliveryLocationActivity.this, calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH), calendar.get(Calendar.DAY_OF_MONTH));
        dpd.setMinDate(calendar);
        dpd.setAccentColor(DeliveryLocationActivity.this.getResources().getColor(R.color.colorPrimary));
    ...
    void onClickSelectTime() {
        ...
        if (DATE.equals("")) {
            showMessage("Please Select Date First");
        } else {
            Calendar calendar = Calendar.getInstance();
    ...
    public void onTimeSet(RadialPickerLayout view, int hourOfDay, int minute, int second) {
        ...
        Log.d("**SCHEDULE_TIME===", TIME);

        locationSession.setScheduleTIme(DATE, TIME);
